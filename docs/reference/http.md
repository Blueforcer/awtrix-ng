...
| `hidden` | what is switched off |

**`hidden` is always required. `order` is optional, and requires `hidden` beside it.** A body without
`hidden` is refused.

| Body | Effect |
|---|---|
| `{"order":[…],"hidden":[…]}` | sets both |
| `{"hidden":[…]}` | switches those off, leaves the order alone |
| `{"order":[…]}` | **400** |
...
* A name in `hidden` stays switched off while the app is away.
...
|---|---|
| 200 | `{"ok":true}` |
| 400 | not valid JSON, a body that is not an object, or one without `hidden` (`invalidJson`) |
| 405 | wrong method - `allowed method(s): PUT` |

```bash
curl -X PUT http://<awtrix-ip>/api/v1/apps/order \
  -H "Content-Type: application/json" \
  -d '{"order":["Time","weather","Time","Date"],"hidden":["Battery"]}'
```
...
its place: the name stays in `order` or `hidden`, listed with `present: false`, and the next push
