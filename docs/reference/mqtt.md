...
| `<P>/cmd/audio/play` | `{"clip"}` \| `{"melody"}` \| `{"rtttl"}` \| `{"builtin"}` \| `{"station"}` \| `{"index"}` \| `{"url"}` | `POST /api/v1/audio/play` |
...
Send **exactly one** key. Sending more than one is refused whole with
`{"ok":false,"error":{"code":"validationFailed","message":"exactly one of \"clip\", \"melody\",
\"rtttl\", \"builtin\", \"station\", \"index\" or \"url\" is allowed","field":"clip"}}` and nothing
plays - no key wins over another, and there is no fallback. Full rules:
[HTTP API - POST /api/v1/audio/play](http.md#post-apiv1audioplay).

| Key | Type | Range | Default | Units | Meaning |
|---|---|---|---|---|---|
| `clip` | string | a stored MP3 | - | - | Play `/CLIPS/<name>.mp3`. |
| `melody` | string | a stored melody | - | - | Play `/MELODIES/<name>.txt`, or a DFPlayer track. |
...
```bash
mosquitto_pub -h broker.local -t 'awtrixNG/cmd/audio/play' -m '{"clip":"beep"}'
...
```json
{"ok":false,"error":{"code":"validationFailed","message":"exactly one of \"clip\", \"melody\", \"rtttl\", \"builtin\", \"station\", \"index\" or \"url\" is required","field":"clip"}}
```
...
`no clip of that name`, and `invalidJson` says `payload` where HTTP says
`request body`.
