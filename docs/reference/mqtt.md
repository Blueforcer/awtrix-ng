...
| `<P>/cmd/apps/order` | `{"order":["Time","weather"],"hidden":["Battery"]}`; `hidden` is required, `order` is optional | `PUT /api/v1/apps/order` |
...
`order` is what runs, in the order it draws; `hidden` is what is switched off. Duplicates in `order`
are allowed. An app named in neither list keeps what it had. `hidden` is always required, `order` is
optional.

```bash
mosquitto_pub -h broker.local -t 'awtrixNG/cmd/apps/order' \
  -m '{"order":["Time","weather","Date"],"hidden":["Battery"]}'
```

`hidden` can be sent on its own, so one app can be switched off without resending the arrangement:
