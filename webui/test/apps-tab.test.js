/* Apps tab: three cards, and what Save sends.
   ...
   order, and `hidden` is exactly what is switched off. Deriving `hidden` from
...
async function run() {
  ...
  assert(JSON.stringify(store.order.hidden) === JSON.stringify([]),
  ...
  assert(store.order.hidden.slice().sort().join(',') === 'Bridge,Date,Doorbell',
    'hidden names every switched-off app, which is what stops a background script');
  assert(!store.order.hidden.includes('co2'),
  ...
  await save();
  assert(!store.order.order.includes('co2') && !store.order.hidden.includes('co2'),
